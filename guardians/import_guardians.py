...
def resolve_ens_names():
    ...
    for row in cur_select.execute(
        "SELECT id, address_or_ens FROM guardians ORDER BY id"
    ):
        ...
        cur_update.execute(
            "UPDATE guardians SET address=?, ens=? WHERE id=?", (address, ens, row[0])
        )
        con.commit()
    con.close()
...
def download_images():
    ...
    for row in cur_select.execute("SELECT id, image_url FROM guardians ORDER BY id"):
        ...
        if image_url:
            ...
            if img_r.ok:
                cur_update.execute(
                    "UPDATE guardians SET image_src=? WHERE id=?",
                    (sqlite3.Binary(img_r.content), id),
                )
        con.commit()
    con.close()
...
def substitute_images():
    ...
    for file in os.listdir(directory):
        ...
        for row in cur_select.execute(
            "SELECT id FROM guardians WHERE name LIKE ? ORDER BY id LIMIT 1",
            [guardian_name],
        ):
            id = row[0]
...
def svg_to_pil(svgbytes):
    ...
    with wand.image.Image(
        blob=buf, format="svg", background=wand.color.Color("transparent")
    ) as image:
...
def convert_images():
    ...
    for row in cur_select.execute(
        "SELECT id, image_url, image_src FROM guardians ORDER BY id"
    ):
        ...
        if image_src:
            ...
            if pil:
                ...
                cur_update.execute(
                    "UPDATE guardians SET image_1x=?, image_2x=?, image_3x=? WHERE id=?",
                    (
                        sqlite3.Binary(png_1x),
                        sqlite3.Binary(png_2x),
                        sqlite3.Binary(png_3x),
                        id,
                    ),
                )
                con.commit()
...
def export_guardians_json():
    ...
    guardians = []
    for row in cur_select.execute(
        "SELECT name, address, ens, reason, contribution FROM guardians ORDER BY name"
    ):
...
def export_guardian_images():
    ...
    cur_select = con.cursor()
    for row in cur_select.execute(
        "SELECT address, image_1x, image_2x, image_3x FROM guardians WHERE image_3x IS NOT NULL ORDER BY address"
    ):
        address, image1, image2, image3 = row
        print(address)
        with open(f"../data/guardians/images/{address}_1x.png", "wb") as f1, open(
            f"../data/guardians/images/{address}_2x.png", "wb"
        ) as f2, open(f"../data/guardians/images/{address}_3x.png", "wb") as f3:
            f1.write(image1)
            f2.write(image2)
            f3.write(image3)
    con.close()
